...
int32_t numLDescents(const CoxGroup& W, const CoxWord& w) {
  auto numDescents = 0;
...
int32_t maxw0(const CoxGroup& W, const CoxWord& w) {
  const auto n = W.rank();
  const auto dw = numLDescents(W, w);
  const auto& t = W.type();
  const auto& typeName = t.name();

  if (typeName == io::String("A")) {
    return choose(dw+1, 2);
  } else if (typeName == io::String("B")) {
    return dw * dw;
  } else if (typeName == io::String("D")) {
    return dw > 3 ? dw*(dw-1) : choose(dw+1, 2);
  } else if (typeName == io::String("E")) {
    const static std::vector<int32_t> m_e = {0, 1, 3, 6, 12, 20, 36, 63, 120};
    return m_e.at(dw);
  } else if (typeName == io::String("F")) {
    const static std::vector<int32_t> m_f = {0, 1, 4, 9, 24};
    return m_f[dw];
  } else if (typeName == io::String("G")) {
    const static std::vector<int32_t> m_g = {0, 1, 6};
    return m_g[dw];
  } else if (typeName == io::String("H")) {
    const static std::vector<int32_t> m_h = {0, 1, 5, 15, 60};
    return m_h[dw];
  } else if (typeName == io::String("I")) {
    return dw == 2 ? n : dw;
  } else {
